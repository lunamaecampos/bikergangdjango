...
def tourdates (request):
    ...
    artisttourset = ArtistProfile.objects.all()
    tour_list = TourDates.objects.order_by('tourdatetime')
    # tour_list = artist.tourdates_set.all()
    # tourid = artistid.tourdates_set.get(id=1)
    # print tourid.tourcity
    # paginator = Paginator(artist, 50) # Show 50 Tourdates per page
    #
    ...
    return render(request, 'bikeradmin/tourdates.html', {'artist':artist, 'artisttourset':artisttourset})
...
def tourSelect(request, artisturlname):
    artist = ArtistProfile.objects.all()
    artistid = ArtistProfile.objects.get(artisturlname=artisturlname)
    id = artistid.id
    tour_list = TourDates.objects.filter(artist=id)
    # tourid = artistid.tourdates_set.get(id=1)
    # print tourid.tourcity
    tour_list = tour_list.order_by('tourdatetime')
    context = {'artist':artist, 'artistid':artistid}
